...
This contract intentionally checks authority, recency and semantic requirements.
It must not make punctuation, heading capitalisation or historical branch wording
part of current product truth.
"""
...
def read(path: Path) -> str:
    require(
        path.is_file(),
        f"Required documentation file is missing: {path.relative_to(PROJECT_ROOT)}",
    )
    return path.read_text(encoding="utf-8")


def normalise(text: str) -> str:
    """Return lowercase words with punctuation and formatting removed."""
...
def require_semantic(text: str, expected: str, message: str) -> None:
    require(normalise(expected) in normalise(text), message)
...
def parse_date(text: str, label: str, document_name: str) -> datetime:
    match = re.search(
        rf"\*\*{re.escape(label)}:\*\*\s+(\d{{1,2}} [A-Za-z]+ \d{{4}})",
        text,
    )
...
def parse_runtime_baseline(text: str, document_name: str) -> tuple[int, str]:
    ...
    require(
        match is not None,
        f"{document_name} does not declare a parseable merged runtime baseline",
    )
...
def main() -> int:
    ...
    for requirement in (
        ...
        require_semantic(
            source_truth,
            requirement,
            f"Source Truth omits required authority rule: {requirement}",
        )
    ...
    # Known stale snapshots must not return as present authority.
    for stale in (
    ...
    # Recovery must be finite and precede broad feature work.
    ...
    for requirement in (
        ...
        require_semantic(
            register,
            requirement,
            f"Regression register omits causal field or mechanism: {requirement}",
        )
    ...
    # Product authority remains Stage One plus Stage Two, with AI subordinate.
    for requirement in (
    ...
    for requirement in (
        ...
        require_semantic(
            architecture,
            requirement,
            f"Supporting architecture omits safety boundary: {requirement}",
        )

    # Present capability must include the latest accepted foundation and limits.
    for requirement in (
